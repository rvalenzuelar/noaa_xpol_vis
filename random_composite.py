...
def rnd_rhi(reps=None, regime=None):
    ...
    if reps is None:
        ...
        frac.where(frac < 1).plot(ax=ax[1], vmin=0, vmax=1)
        ...
        m = mean.where(rho<28).plot.contourf(ax=ax[0],
                                levels=16, vmin=0, vmax=30,
                                add_labels=True,
                                cmap=cmap_rhi)
        f = frac.where(rho<28).plot.contourf(ax=ax[1], cmap='inferno',
                                levels=9,
                                vmin=20, vmax=100)
...
def rnd_ppi(reps=None, regime=None):
    ...
    if reps is None:
        ...
        ''' make plots '''
        m=mean.where(rho<55).plot.contourf(ax=ax[0], levels=16,
                                           vmin=-30, vmax=30,
                                            cmap=cmap_ppi)
        f=frac.where(rho<55).plot.contourf(ax=ax[1], vmin=20,
                                           vmax=100,
                           cmap='inferno',
                                levels=9,)
...
if __name__ == '__main__':
    ...
    # rnd_rhi(reps=2, regime='tta')
    # rnd_rhi(reps=2, regime='ntta')
